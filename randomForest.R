...
xtest2=lapply(xtest,as.factor)
xtest2=as.data.frame(xtest2)




pepa=predict(model_glm,res1)


confusionMatrix(pepa,as.factor(test[,1]))
...
Survivedpepa<- ((as.numeric(pepa)-1)+(as.numeric(pepa2)-1))/2
Survivedpepa<-ifelse(Survivedpepa<0.5,0,1)


y<-data.frame(Survivedpepa,pepa,pepa2,test)


write.csv(y,"afterensa.csv",row.names = F)
...
write.csv(y,"sendensa.csv",row.names = F)
